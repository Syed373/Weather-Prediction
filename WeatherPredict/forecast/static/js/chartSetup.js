document.addEventListener('DOMContentLoaded', () => {
    const chartElement = document.getElementById('chart');
    if (!chartElement) {
        console.error('Canvas Element not found.');
        return;
    }

    const ctx = chartElement.getContext('2d');

    const gradient = ctx.createLinearGradient(0, -10, 0, 100);
    gradient.addColorStop(0, 'rgba(250, 0, 0, 1)');
    gradient.addColorStop(1, 'rgba(136, 255, 0, 1)'); 

    const forecastItems = document.querySelectorAll('.forecast-item'); 

    const temps = [];
    const times = [];
    const hums = [];

    forecastItems.forEach((item) => {
        const time = item.querySelector('.forecast-time')?.textContent || "N/A";
        const tempEl = item.querySelector('.forecast-temperatureValue');
        const humE = item.querySelector('.forecast-humidityvalue').textContent;

        if (tempEl && humE && time) {
            const temp = tempEl.textContent;
            times.push(time);
            temps.push(temp);
            hums.push(humE);

        } else {
            console.warn("Temperature or Time or Humidity element not found in:", item);
        }
    });

    if (temps.length === 0 || times.length === 0 ||hums.length === 0) {
        console.error('Temp or Time or Hums values are missing.');
        return;
    }

    new Chart(ctx, {
        type: 'line',
        data: {
            labels: times,
            datasets: [
                {
                    label: 'Celsius Degrees',
                    data: temps,
                    borderColor: gradient,
                    borderWidth: 2,
                    tension: 0.4,
                    pointRadius: 2,
                },
            ],
        },
        options: {
            plugins: {
                legend: {
                    display: false,
                },
            },
            scales: {
                x: {
                    display: false,
                    grid: {
                        drawOnChartArea: false,
                    },
                },
                y: {
                    display: false,
                    grid: {
                        drawOnChartArea: false,
                    },
                },
            },
            animation: {
                duration: 750,
            },
        },
    });
});
